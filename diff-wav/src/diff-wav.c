...
static const char* appname = "diff-wav";
...
bool blk_conv_to_double(double** to, void** from, uint16_t from_format, size_t from_sample_size, int nch, int blksz) {
    ...
    } else if (from_format == WAV_FORMAT_IEEE_FLOAT) {
...
bool diff_file(char* in1name, char* in2name, char* outname) {
    WavFile* in1 = wav_open(in1name, "r");
    WavFile* in2 = wav_open(in2name, "r");
    WavFile* out = wav_open(outname, "w");
    size_t   readlen1, readlen2, readlen;
    int      nch = wav_get_num_channels(in1);
    void**   buf1;
    void**   buf2;
    double** buf1_double;
    double** buf2_double;
    ...
    printf("  format: %#06x\n", wav_get_format(in1));
    printf("  sample size: %zu\n", wav_get_sample_size(in1));
    printf("  valid bits per sample: %u\n", wav_get_valid_bits_per_sample(in1));
    printf("  sample rate: %u\n", wav_get_sample_rate(in1));
    printf("  length: %zu\n", wav_get_length(in1));
    printf("input file 1: %s\n", in2name);
    printf("  format: %#06x\n", wav_get_format(in2));
    printf("  sample size: %zu\n", wav_get_sample_size(in2));
    printf("  valid bits per sample: %u\n", wav_get_valid_bits_per_sample(in2));
    printf("  sample rate: %u\n", wav_get_sample_rate(in2));
    printf("  length: %zu\n", wav_get_length(in2));
    ...
    if (nch != wav_get_num_channels(in2) || wav_get_sample_rate(in1) != wav_get_sample_rate(in2)) {
        printf("  diff failed!\n");
        return false;
    }
    ...
    for (i = 0; i < nch; ++i) {
        buf1[i]        = malloc(container_size(wav_get_sample_size(in1)) * BLOCKSIZE);
        buf2[i]        = malloc(container_size(wav_get_sample_size(in2)) * BLOCKSIZE);
    ...
    wav_set_format(out, WAV_FORMAT_IEEE_FLOAT);
    wav_set_sample_size(out, 8);
    wav_set_sample_rate(out, wav_get_sample_rate(in1));
    wav_set_num_channels(out, nch);
    printf("output file: %s\n", outname);

    do {
        readlen1 = wav_read(buf1, BLOCKSIZE, in1);
        readlen2 = wav_read(buf2, BLOCKSIZE, in2);

        //system("pause");

        readlen = (readlen1 < readlen2) ? readlen1 : readlen2;

        blk_conv_to_double(buf1_double, buf1, wav_get_format(in1), wav_get_sample_size(in1), nch, readlen);
        blk_conv_to_double(buf2_double, buf2, wav_get_format(in2), wav_get_sample_size(in2), nch, readlen);

        for (i = 0; i < nch; ++i) {
            for (j = 0; (size_t)j < readlen; ++j) {
        ...
        wav_write((void**)bufout, readlen, out);
